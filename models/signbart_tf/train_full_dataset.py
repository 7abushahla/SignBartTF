...
def main():
    ...
    # Also patch the eval_loader creation in main() to handle missing test split
    original_main = main_functional.main
    def patched_main_functional(args):
        """Wrapped main function that handles missing test split."""
        # We'll let it run normally, but we need to handle the eval_loader creation
        # The eval_loader creation happens inside main(), so we need to patch SignDataset
        # or catch the error. Actually, let's just patch the part that creates eval_loader.
        
        # Store original SignDataset
        from dataset import SignDataset as OriginalSignDataset
        
        import json
        
        class SafeSignDataset(OriginalSignDataset):
            """SignDataset that handles missing splits gracefully."""
            def __init__(self, root, split, shuffle=True, joint_idxs=None, augment=False):
                # Check if split directory exists
                split_dir = os.path.join(root, split)
                if not os.path.exists(split_dir):
                    # If split doesn't exist and it's "test", create empty dataset
                    if split == "test":
                        # Create a dummy dataset that will be skipped
                        self.root = root
                        self.split = split
                        ...
                        self.list_key = []  # Empty list
                        return
                # Otherwise, use original behavior
                super().__init__(root, split, shuffle, joint_idxs, augment)
        
        # Temporarily replace SignDataset
        import dataset
        original_sign_dataset = dataset.SignDataset
        dataset.SignDataset = SafeSignDataset
        main_functional.SignDataset = SafeSignDataset
        
        try:
            result = original_main(args)
        finally:
            # Restore original
            dataset.SignDataset = original_sign_dataset
            main_functional.SignDataset = original_sign_dataset
        
        return result
    
    main_functional.main = patched_main_functional
