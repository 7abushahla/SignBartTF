...
AUTOMATICALLY CREATES METADATA FILES (label2id.json, id2label.json)
"""
...
def create_metadata_files(gesture_dirs, loso_path):
    """Create label2id.json and id2label.json from gesture directories."""
    # Create mappings: G01 -> 0, G02 -> 1, etc.
    label2id = {}
    id2label = {}
    
    for idx, gesture in enumerate(sorted(gesture_dirs)):
        label2id[gesture] = idx
        id2label[str(idx)] = gesture
    
    # Save label2id.json
    label2id_path = os.path.join(loso_path, "label2id.json")
    with open(label2id_path, 'w') as f:
        json.dump(label2id, f, indent=2)
    
    # Save id2label.json
    id2label_path = os.path.join(loso_path, "id2label.json")
    with open(id2label_path, 'w') as f:
        json.dump(id2label, f, indent=2)
    
    return ['label2id.json', 'id2label.json']

def copy_metadata_files(data_source, loso_path):
    """Copy existing metadata files (vocab.json, config.json) to LOSO directory."""
    metadata_files = ['vocab.json', 'config.json']
    copied_files = []
...
def create_loso_split(base_data_path, holdout_user, train_users, force=False):
    ...
    # Get gesture directories (G01, G02, etc.) BEFORE creating LOSO directory
    gesture_dirs = []
    ...
    # Create LOSO directory
    os.makedirs(loso_path, exist_ok=True)
    
    # Create metadata files (label2id.json, id2label.json) from gesture directories
    print(f"  Creating metadata files...")
    created_files = create_metadata_files(gesture_dirs, loso_path)
    print(f"    ✓ Created: {', '.join(created_files)}")
    
    # Copy any existing metadata files (vocab.json, config.json)
    ...
    label2id_exists = os.path.exists(os.path.join(loso_path, "label2id.json"))
    id2label_exists = os.path.exists(os.path.join(loso_path, "id2label.json"))
    
    if not label2id_exists or not id2label_exists:
        print(f"    ✗ ERROR: Metadata files not created properly")
        return False
